...
static inline http_request_t http_parse_request(const char *buf, size_t len) {
    ...
    if (len < 14) return req; // Minimum: "GET / HTTP/1.1"
    ...
    if (method_len >= sizeof(req.method)) return req;
    ...
    if (path_len >= sizeof(req.path)) return req;
...
static inline int http_path_get_int(const char *path, const char *prefix) {
    ...
    for (int i = 0; num_str[i] != '\0'; i++) {
        if (num_str[i] < '0' || num_str[i] > '9') {
            ...
            if (i == 0) return -1; // No digits at all
